...
class MotionModel:
    ...
    def __init__(self):
        ...
        tmu, tsig = 1, 0.1
        self.points = 1000
        self.noise = np.random.normal([xmu, ymu, tmu], [xsig, ysig, tsig], points)
    ...
    def evaluate(self, particles, odometry):
        ...
        for i in range(length(particles)):
            ...
            particle_pose = self.vec2pose(p[i])
            new_pose = particle_pose*odom_pose
    ...
    def pose2vec(self, pose):
        ...
        theta = np.arccos(pose[0,0]) * (sign(np.arcsin(pose[0,0])))
        p = np.array([pose[0,2], pose[0,1], pose])
